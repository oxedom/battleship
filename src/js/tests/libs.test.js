const { XYtoIndex, checkLegalMove, getShipArray, indexToXY } = require('../libs')
...
test('INDEX TO XY', () => {
    ...
    expect(indexToXY(5)).toEqual({ x: 5, y: 1 });
})

test('INDEX TO XY', () => {

    expect(indexToXY(10)).toEqual({ x: 10, y: 1 });
})

test('INDEX TO XY', () => {

    expect(indexToXY(33)).toEqual({ x: 3, y: 4 });
}) 